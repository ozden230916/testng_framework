...
public class _12_Actions_Class extends Base{
   ...
   /*
Go Go to https://www.techglobalschool.com/
 ...
 */
    ...
    public void rightClickPractice(){
        driver.get("https://www.techglobalschool.com/");
        WebElement registerForIntroLink = driver.findElement(By.cssSelector("#comp-kvjohd5t5>a"));
        actions.contextClick(registerForIntroLink);
...
Validate “Drop here” text is replaced with “Dropped!”
    ...
    public void sendKeysToAnElement(){
        ...
        actions.sendKeys(inputBox, "Hello").perform();
    ...
    public void etsyTest(){
        ...
        String[] categoriesList = {"Accessories","Bags & Purses", "Necklaces", "Rings", "Earrings", "Bracelets","Body Jewellery", "All Jewellery"};
        actions.moveToElement(etsySearchPage.mainHeaderLinks.get(1)).perform();
        Waiter.pause(2);

        for (int i = 0; i < categoriesList.length; i++) {
            Waiter.waitUntilTextToBePresentInElement(driver, 30, etsySearchPage.jewelryAndAccessoriesItems.get(i), categoriesList[i]);
            Assert.assertEquals(etsySearchPage.jewelryAndAccessoriesItems.get(i).getText(), categoriesList[i]);
        }
